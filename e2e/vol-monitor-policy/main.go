...
const (
	...
	VolumeFromPVCFC FetchCommand = `kubectl get pvc vol-percona-monitor-claim -o jsonpath='{.spec.volumeName}' --namespace=default`
...
const (
	...
	PerconaStorageClassVC VerifyCommand = `kubectl get sc sc-percona-monitor -o name`
...
const (
	...
	// storage class
	PerconaStorageClassDC DisplayCommand = `kubectl get sc sc-percona-monitor -o yaml`
	...
	PerconaPVCDC DisplayCommand = `kubectl get pvc vol-percona-monitor-claim -o yaml`
)
...
const (	
	...
	PerconaAppDELC DeleteCommand = `kubectl delete po percona-monitor`

	// PerconaPVCDELC will delete the percona pvc
	PerconaPVCDELC DeleteCommand = `kubectl delete pvc vol-percona-monitor-claim`

	// PerconaSCDELC will delete the percona sc
	PerconaSCDELC DeleteCommand = `kubectl delete sc sc-percona-monitor`
)
...
parameters:
  ...
  openebs.io/volume-monitor: "na: openebs/m-exporter:ci"
EOF`
...
spec:
  ...
  - resources:
    ...
    - mountPath: /var/lib/mysql
      name: vol-percona-monitor
  volumes:
  - name: vol-percona-monitor
    persistentVolumeClaim:
      claimName: vol-percona-monitor-claim
---
kind: PersistentVolumeClaim
apiVersion: v1
metadata:
  name: vol-percona-monitor-claim
spec:
  storageClassName: sc-percona-monitor
  accessModes:
    - ReadWriteOnce
  resources:
    requests:
